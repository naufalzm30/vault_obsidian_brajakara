...
Tracking semua project aktif Brajakara (backend + frontend). Path lokal mengacu pada mesin **tower** (primary dev PC).
...
- **salazar** (laptop) — TIDAK ada project lokal, cuma vault + Claude Code
- **tower** (PC) — semua project di `~/` langsung (flat layout)
- Path di kolom "Path (tower)" adalah standar di tower — mesin lain bisa beda
